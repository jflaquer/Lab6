...
public class Rule {
	...
	public Rule(int RuleID) {
		for (GameRuleDomainModel gr : GameRuleBLL.getRules()) {
			this.setRuleID(gr.getRULEID());
			this.MaxNumberOfPlayers = gr.getMAXNUMBEROFPLAYERS();
			this.PlayerNumberOfCards = gr.getPLAYERNUMBEROFCARDS();
			this.NumberOfJokers = gr.getNUMBEROFJOKERS();
			this.PlayerCardsMin = gr.getPLAYERCARDSMIN();
			this.PlayerCardsMax = gr.getPLAYERCARDSMAX();
			this.CommunityCardsMin = gr.getCOMMUNITYCARDSMIN();
			this.CommunityCardsMax = gr.getCOMMUNITYCARDSMAX();
			this.PossibleHandCombinations = gr.getPOSSIBLEHANDCOMBINATIONS();
